        ...
        <main>
            ...
            if (isset($_GET["category"])) {
                ...
                if (isset($_GET["search"]) && $_GET["search"] !== "") {
                    $produits = $db->query('SELECT * FROM products WHERE (' . $c_query . ') AND Name LIKE "%' . $_GET["search"] . '%" AND (Vendor LIKE "NULL")');
                } else {
                    $produits = $db->query('SELECT * FROM products WHERE (' . $c_query . ') AND (Vendor LIKE "NULL")');
                }
            } else if (isset($_GET["search"]) && $_GET["search"] !== "") {
                $produits = $db->query('SELECT * FROM products WHERE Name LIKE "%' . $_GET["search"] . '%" AND (Vendor LIKE "NULL")');
            } else {
                $produits = $db->query('SELECT * FROM products WHERE Vendor LIKE "NULL"');
            }
...
<footer class="footer">
    <p>&copy; <?php echo date("Y"); ?> Prestige - Vente de montres de luxe</p>
    <p>Créé par Sajed et Abdoulaye - Paris Ynov Campus</p>
    <p><a href="https://www.ynov.com/campus/paris/">Visitez notre campus</a></p>
</footer>
</body>
